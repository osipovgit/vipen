...
- 💾 **Бэкапы** — ежедневный снимок БД на сервере с ротацией, off-host копия `x-ui.db` на локальную машину, восстановление на новом сервере.
...
- 🔔 **Оповещения** — push в Telegram при поломке и восстановлении VPN (только при смене состояния), ежедневный отчёт при проблемах гипервизора (один суммирующий за сутки).
...
**Оповещения в Telegram — в [`docs/alerting.md`](docs/alerting.md):** что попадает в `FAIL`, а что в `WARN`, создание бота, хранение токена в Ansible Vault.
